...
char analisis(short *segmento1,short *segmento2,short valorMaximo){
	...
	for(int i=1,j=0;i<16;i=i+2,j++){
		if((segmento1[i]>(valorMaximo) && segmento2[i]==0 )| (segmento1[i]>(valorMaximo) && segmento2[i]>(valorMaximo))|(segmento2[i]>(valorMaximo)&& segmento1[i]==0 )){
			aux=pow(2,7-j);
			a=a|aux;
		}
	}
...
void TDF(short *muestrasAudio,int numMuestrasAudio){
